...
export function ProductGallery({ asset, boxColorHex, foilColorHex, ribbonColorHex }: Props) {
  const [activeIndex, setActiveIndex] = useState(0);
  const [showVideo, setShowVideo] = useState(false);
  const videoRef = useRef<HTMLVideoElement>(null);

  const videoUrl = `${import.meta.env.BASE_URL}video/box-orbit.mp4`;
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          alignItems: 'center',
          gap: 6,
          background: asset.status === 'ready' ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.95)',
          padding: '6px 12px',
          fontSize: 11,
          fontWeight: 400,
          letterSpacing: 0.5,
        ...
        <div style={{
          ...
          {showVideo ? '360° Product Rotation — Google Veo' : (imageLabels[activeIndex] || 'Product View')}
        </div>
      </div>
